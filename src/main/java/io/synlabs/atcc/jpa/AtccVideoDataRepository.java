...
public interface AtccVideoDataRepository extends JpaRepository<AtccVideoData, Long> {
    ...
    @Query(value = "SELECT id, filename, time_stamp as timestamp FROM atcc_video_data where (time_stamp + offset) <= ?1 and (time_stamp + 120 + offset) > ?1 and feed = ?2 limit 1",nativeQuery = true)
